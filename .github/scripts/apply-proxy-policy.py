...
The HTTP response body is capped''',
...
The HTTP response body is capped''',
    "proxy trust-boundary documentation",
)

Path("tests/proxy_boundary.rs").write_text(r'''use assert_cmd::Command;
...
fn model_request_ignores_environment_proxies() {
    ...
    endpoint.set_nonblocking(true).expect("endpoint nonblocking");
