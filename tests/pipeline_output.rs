...
fn model_server() -> (String, thread::JoinHandle<()>) {
...
#[test]
fn json_dry_run_keeps_stdout_machine_readable_and_usage_on_stderr() {
    ...
    assert!(!String::from_utf8_lossy(&output.stdout).contains("Model usage:"));

    endpoint.join().expect("model endpoint thread");
}

#[test]
fn json_without_dry_run_is_rejected_before_repository_discovery() {
    let outside_repo = tempdir().expect("temporary directory");
    let mut command = Command::new(env!("CARGO_BIN_EXE_git-autocommit"));
    command
